...
from astarthought.evals.batch import Pipeline, init_engine_from_config
from astarthought.evals.batch.env_config import EnvConfig
from astarthought.evals.batch.workload import EvalWorkload
from astarthought.evals.common.entities import (
    Backend,
    BackendParameters,
...
from astarthought.evals.models import ModelConfig
from astarthought.evals.tasks import (
    ConversationType,
    NUMINATaskHandler,
    TaskHandler,
)
from astarthought.evals.util.metrics import pass_at_k
from astarthought.evals.util.response import Response, SingleParsedResponse
from astarthought.evals.util.results import SummaryResults, save_summary
...
def generate_and_score(
    ...
    **kwargs,
    ...
    logger.info(f"Accuracy: {round(accuracy * 100, 1)}")
    ...
    avg_completion_tokens = round(total_completion_tokens / num_responses_total, 2) if total_completion_tokens else 0
    logger.info(f"Average Completion Tokens: {avg_completion_tokens}")
...
def generate_and_save(
    ...
    **kwargs,
    ...
    avg_completion_tokens = round(total_completion_tokens / num_responses_total, 3) if total_completion_tokens else 0
    logger.info(f"Average Completion Tokens: {avg_completion_tokens}")

...
class PromptCompressor:
    ...
    def load_model(
        ...
        self.device = (
            device_map
            if any(key in device_map for key in ["cuda", "cpu", "mps", "cuda:0", "cuda:1", "cuda:2", "cuda:3", "cuda:4", "cuda:5", "cuda:6", "cuda:7"])
            else "cuda"
        )
        if "cuda" in device_map or "cpu" in device_map:
            model = MODEL_CLASS.from_pretrained(
                ...
                torch_dtype=model_config.pop(
                    "torch_dtype", "auto" if device_map in ["cuda", "cuda:0", "cuda:1", "cuda:2", "cuda:3", "cuda:4", "cuda:5", "cuda:6", "cuda:7"] else torch.float32
                ),
    ...
    def get_rank_results(
        ...
        def get_distance_longcot(corpus, question, solution, alpha):
            bis = self.get_bidirectional_importance_score(
            ...
            sort_direct = 1 if condition_in_question == "none" else -1
            sent_sort = sorted(
                enumerate(bis), key=lambda x: sort_direct * x[1]
            )
            return sent_sort

        method = None
        if rank_method == "long_cot":
            method = get_distance_longcot
            return method(context, question, solution, alpha)
    ...
    def get_ppl_and_attnscore(
        ...
        if condition_mode == "before":
            ...
            loss = loss[condition_pos_id:]
        ppl = loss.mean() if granularity == "sentence" else loss

        ## get Attention Scores
        last_layer_attention = response.attentions[-1]
        thought_start, thought_end = (0, condition_pos_id + 1) if condition_mode == "after" else (condition_pos_id + 1, end)
        solution_start, solution_end = (condition_pos_id + 1, end) if condition_mode == "after" else (0, condition_pos_id + 1)
        solution_to_thought_attention = last_layer_attention[:, :, solution_start:solution_end, thought_start:thought_end]
    ...
    def get_condition_ppl_and_attnscore(
        ...
        if condition_in_question == "none":
            return self.get_ppl_and_attnscore(text, granularity=granularity)
        elif condition_in_question == "before":
            return self.get_ppl_and_attnscore(
                question + text,
                granularity=granularity,
                condition_mode="after",
                condition_pos_id=self.get_token_length(question) - 1,
    ...
    def get_bidirectional_importance_score(
        self,
        ...
        question_results = [
            self.get_condition_ppl_and_attnscore(
                thought_step,
                question + " We can get the solution to this question in the given thought step.",
                condition_in_question
            )
            for thought_step in thought_steps
        ]
        question_ppl = [result[0].cpu().item() for result in question_results]
        question_attn_score = [result[1].cpu().item() for result in question_results]
        solution_results = [
            self.get_condition_ppl_and_attnscore(
                thought_step,
                solution + " We can get this solution in the given thought step.",
                condition_in_question
            )
            for thought_step in thought_steps
        ]
        solution_ppl = [result[0].cpu().item() for result in solution_results]
        solution_attn_score = [result[1].cpu().item() for result in solution_results]
        bis = [
            ((1 - alpha) * question_attn_score[i] + alpha * solution_attn_score[i]) / ((1 - alpha) * question_ppl[i] + alpha * solution_ppl[i])
            for i in range(len(thought_steps))
        ]

        return bis
    
    def get_bis_sentence_sort(
        self,
        context: str = "",
        question: str = "",
        solution: str = "",
        ...
        thought_steps = [item.strip() for item in context.split("\n\n") if item]
        bis = self.get_bidirectional_importance_score(
            thought_steps=thought_steps,
            question=question,
            solution=solution,

...
def _get_dataset_processor(
    ...
    if stage == "pt":
        ...
        if data_args.packing:
            ...
            if model_args.astar_thought_latent:
                from .processor import AStarThoughtSupervisedDatasetProcessor
                dataset_processor_class = AStarThoughtSupervisedDatasetProcessor
            else:
...
def _get_preprocessed_dataset(
    ...
    dataset_processor = _get_dataset_processor(
        model_args, data_args, stage, template, tokenizer, processor, do_generate=(training_args.predict_with_generate and is_eval)
    )
    ...
    with training_args.main_process_first(desc="pre-process dataset", local=(not data_args.data_shared_file_system)):
        ...
        if "train" in train_dict:
            train_dict["train"] = _get_preprocessed_dataset(
                train_dict["train"], model_args, data_args, training_args, stage, template, tokenizer, processor, is_eval=False
            )

        for key in eval_dict:
            eval_dict[key] = _get_preprocessed_dataset(
                eval_dict[key], model_args, data_args, training_args, stage, template, tokenizer, processor, is_eval=True
            )

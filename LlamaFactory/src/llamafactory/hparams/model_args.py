...
class ModelArguments(
    ...
    ## [NEW] A*-Thought-Latent
    astar_thought_latent: bool = field(
        default=False,
        metadata={"help": "Whether or not to use A*-Thought-latent."},
    )
    latent_begin_token: str = field(
        default="<latent>",
        metadata={"help": "The a*-thought-special tag of the beginning of latent token."}
    )
    latent_end_token: str = field(
        default="</latent>",
        metadata={"help": "The a*-thought-special tag of the ending of latent token."}
    )
    latent_tokens: str = field(
        default="<latent_1>,<latent_2>,<latent_3>,<latent_4>,<latent_5>,<latent_6>,<latent_7>,<latent_8>",
        metadata={"help": "The latent tokens used in latent token prediction, separated by commas."}
    )
    structure_weight: float = field(
        default=1.0,
        metadata={"help": "The weight for the structure loss on latent token prediction."}
    )
    max_latent_len: int = field(
        default=4,
        metadata={"help": "The maximum length of the latent token sequence."}
    )
    max_latent_count: int = field(
        default=32,
        metadata={"help": "The maximum count of latent spans in the whole sequence."}
    )
